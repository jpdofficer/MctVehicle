...
#Test if the configuation file Exist


#This is an example of how this is run
#$mctInfo = [MctVehicle]::new("10.5.61.1") #create a new MctVehicle object with its ip address and assign it to a varible. 
#$mctInfo.setInterface("Ethernet 4")
#$mctInfo.autoInterface() #in some instances the interface used to connect is named something other than Ethernet. This allows you to specify the interface. For example one car was name Ethernet 3
#$mctInfo.autoUserName() #Call AutoName
#$mctInfo.autoComputerName() #call computer name
#$mctInfo.autoComputerIP() #call computer ip
#$mctInfo.autoComputerMacAddress() #call mac address
#$mctInfo.autoRouterMac() # This gets the ROuters Mac
#$mctInfo.setRouterMac("2a3044519CCE")
#$mctInfo.autoRouterState() #call router state
#$mctInfo.setCSVPath("\\fs4\d\Documentation\GPD-SurvivalGuide\MctAware\routers.csv") #set the exported cvs path so that it can associate the router mac with the documented mac
#$mctInfo.setMctCSVPath("\\fs4\d\Documentation\GPD-SurvivalGuide\MctAware\vehicles.csv")
#$mctInfo.getInterface()

#$routerInventory = Import-Csv -Path $mctInfo.getCSVPath() #Import the router.csv file
#if statemnet if mac does match any router macs save it to file

#if the routerInventory mac matches the $mctInfo.mac
#if(($routerInventory.mac -match $mctInfo.getRouterMac() )) 
#{
    #we set the property name where the mac recorded in the csv file is eq to the one stored in the class
#    $mctInfo.setRouterName( ($routerInventory | Where-Object {$_.mac -eq  $mctInfo.getRouterMac()} | Select-Object name).name )
#    $mctInfo.setRouterSerial(($routerInventory | Where-Object {$_.mac -eq  $mctInfo.getRouterMac()} | Select-Object serial_number).serial_number)

#}
#elseif(-not($routerInventory.mac -eq $mctInfo.getRouterMac() )) #if it does documented mac and routermac does not match place a N/S in name field
# {
    #if not we place not assigned in the value
#    $mctInfo.setRouterName("Not Assigned")

#}

#export this to a new csv file
#$mctInfo | Export-Csv -Path "\\fs4\d\Documentation\GPD-SurvivalGuide\MctAware\vehicles.csv" -Append -NoTypeInformation -Force 